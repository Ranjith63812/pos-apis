...
import { LayoutDashboard, Plus, ChevronDown, Tags } from 'lucide-react';
...
export default function Categories() {
  ...
  const handleSave = async (formData) => {
    if (editItem) {
      await updateCategory(editItem.category_id, formData);
    } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert('Failed to delete category. It may be linked to products.');
    }
  };

  const columns = [
    { key: 'category_name', label: 'Category Name', render: r => <span className="fw-bold text-primary">{r.category_name}</span> },
  ...
  const fields = [
    { key: 'category_name', label: 'Category Name', required: true, placeholder: 'e.g. Electronics' },
    { key: 'category_code', label: 'Category Code', placeholder: 'e.g. ELEC' },
    ...
    <>
      <section className="content-header p-3 pb-0 d-flex justify-content-between align-items-center mb-3">
        <h1 className="m-0 fs-3 fw-normal" style={{ color: '#333' }}>
          Categories List <small className="text-muted fs-6 ms-2">Manage product classifications</small>
        </h1>
          ...
          <div className="box-body p-4 bg-white">
            <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
              <div className="d-flex align-items-center gap-2">
                ...
                <h5 className="mb-0 text-muted fs-6">Categories Data</h5>
              </div>
