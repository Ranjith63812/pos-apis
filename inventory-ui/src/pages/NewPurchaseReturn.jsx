...
export default function NewPurchaseReturn() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [purchaseRes, prodRes] = await Promise.all([getPurchases(), getProducts()]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!header.purchase_id) { alert('Please select an Original Purchase Invoice'); return; }
    ...
    <>
      <section className="content-header p-3 pb-0 d-flex justify-content-between align-items-center mb-3">
        <h1 className="m-0 fs-3 fw-normal" style={{ color: '#333' }}>
          New Purchase Return <small className="text-muted fs-6 ms-2">Process Supplier Return</small>
        </h1>
      </section>
          ...
          <div className="box box-primary rounded-0 shadow-sm mb-4">
            <div className="box-body p-4 bg-white row g-3">
              <div className="col-md-4">
                <label className="fw-bold mb-1" style={{ fontSize: '13px' }}>Original Purchase Invoice *</label>
                <select className="form-select rounded-0" style={{ fontSize: '14px' }}
                  value={header.purchase_id} onChange={e => setHeader({ ...header, purchase_id: e.target.value })} required>
                  <option value="">- Select Invoice -</option>
                  {purchases.map(p => <option key={p.purchase_id} value={p.purchase_id}>{p.purchase_code} - {p.supplier_name}</option>)}
                </select>
              </div>
              <div className="col-md-4">
              ...
              <div className="col-md-4">
                ...
                <input type="text" className="form-control rounded-0" style={{ fontSize: '14px' }} placeholder="e.g. PRET-001"
            ...
            <div className="box-body p-4 bg-white">
              ...
              <div className="table-responsive">
                <table className="table table-bordered table-striped">
                  ...
                  <tbody>
                    {items.length === 0 ? (
                      <tr><td colSpan="7" className="text-center py-4 text-muted"><Box size={30} className="mb-2 opacity-50 d-block mx-auto" />Please select products to return to supplier</td></tr>
                    ) : items.map((item, i) => {
                ...
                <div className="box-body p-4 bg-white">
                  <div className="mb-3 row">
                    ...
                    <div className="col-sm-8">
                       <textarea className="form-control rounded-0" rows="3" placeholder="Reason for supplier return..."
            ...
            <div className="col-md-6">
              <div className="box box-primary rounded-0 shadow-sm mb-4">
                ...
                <div className="box-body p-4 bg-white">
                  ...
                  <div className="mb-3 row">
                    <label className="col-sm-6 col-form-label fw-bold text-danger">Refund Amount Received</label>
                  ...
                  <div className="mt-4 pt-3 border-top text-center">
                    ...
                    <button type="button" className="btn btn-warning rounded-0 px-4 py-2" onClick={() => navigate('/purchase-returns')}>
