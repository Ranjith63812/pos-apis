...
import { LayoutDashboard, Plus, ChevronDown, MapPin } from 'lucide-react';
...
export default function States() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [resS, resC] = await Promise.all([getStates(), getCountries()]);
      setData(resS.data?.data || resS.data || []);
      setCountries(resC.data?.data || resC.data || []);
    } catch (err) {
  ...
  const handleSave = async (formData) => {
    if (editItem) {
      await updateState(editItem.state_id, formData);
    } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteState(deleteTarget.state_id);
  ...
  const columns = [
    { key: 'state_name', label: 'State Name', render: r => <span className="fw-bold text-primary">{r.state_name}</span> },
  ...
  const fields = [
    { key: 'state_name', label: 'State Name', required: true, placeholder: 'e.g. Maharashtra' },
    { 
      key: 'country_id', 
      label: 'Country', 
      type: 'select', 
    ...
    <>
      <section className="content-header p-3 pb-0 d-flex justify-content-between align-items-center mb-3">
        <h1 className="m-0 fs-3 fw-normal" style={{ color: '#333' }}>
          States List <small className="text-muted fs-6 ms-2">Manage provinces/states</small>
        </h1>
          ...
          <div className="box-body p-4 bg-white">
            <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
              <div className="d-flex align-items-center gap-2">
                ...
                <h5 className="mb-0 text-muted fs-6">States Data</h5>
              </div>
