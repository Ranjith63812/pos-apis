...
import { getTaxes, createTax, updateTax, deleteTax } from '../api/taxApi';
import { LayoutDashboard, Plus, ChevronDown, Percent } from 'lucide-react';
...
export default function Taxes() {
  ...
  const handleSave = async (formData) => {
    if (editItem) {
      await updateTax(editItem.tax_id, formData);
    } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteTax(deleteTarget.tax_id);
  ...
  const columns = [
    { key: 'tax_name', label: 'Tax Name', render: r => <span className="fw-bold text-primary">{r.tax_name}</span> },
    { key: 'tax_percentage', label: 'Tax Percentage', render: r => `${r.tax_percentage ?? 0}%` },
  ...
  const fields = [
    { key: 'tax_name', label: 'Tax Name', required: true, placeholder: 'e.g. GST 18%' },
    { key: 'tax_percentage', label: 'Tax Percentage (%)', type: 'number', required: true, placeholder: 'e.g. 18' },
    ...
    <>
      <section className="content-header p-3 pb-0 d-flex justify-content-between align-items-center mb-3">
        <h1 className="m-0 fs-3 fw-normal" style={{ color: '#333' }}>
          Taxes List <small className="text-muted fs-6 ms-2">Manage tax rates</small>
        </h1>
        <ol className="breadcrumb m-0 bg-transparent p-0 float-end" style={{ fontSize: '13px' }}>
          ...
          <li className="breadcrumb-item active">Tax List</li>
        </ol>
      </section>
          ...
          <div className="box-body p-4 bg-white">
            <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
              <div className="d-flex align-items-center gap-2">
                <Percent size={20} className="text-muted" />
                <h5 className="mb-0 text-muted fs-6">Taxes Data</h5>
              </div>
      ...
      <ConfirmDelete 
        show={!!deleteTarget} 
        itemName={deleteTarget?.tax_name} 
